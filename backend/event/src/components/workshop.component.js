...
function WorkshopList (props) {
  ...
  useEffect(() => {
    if(props.name==="up")
      {
      fetch(`https://cn-events.herokuapp.com/events/workshop/up?page=${pageNumber}`)
      ...
      {
      fetch(`https://cn-events.herokuapp.com/events/workshop/ar?page=${pageNumber}`)
      .then(response => response.json())
      .then(({totalPages, events})=>{
        setEvents(events);
        setNumberOfPages(totalPages);
      ...
      {
      fetch(`https://cn-events.herokuapp.com/events/workshop/fav?page=${pageNumber}`)
      .then(response => response.json())
      .then(({totalPages, events})=>{
        setEvents(events);
        setNumberOfPages(totalPages);
