...
function CodingList (props) {
  ...
  useEffect(() => {
    if(props.name==="up")
      {
      fetch(`https://cn-events.herokuapp.com/events/coding/up?page=${pageNumber}`)
      ...
      {
      fetch(`https://cn-events.herokuapp.com/events/coding/ar?page=${pageNumber}`)
      .then(response => response.json())
      .then(({totalPages, events})=>{
        setEvents(events);
        setNumberOfPages(totalPages);
      ...
      {
      fetch(`https://cn-events.herokuapp.com/events/coding/fav?page=${pageNumber}`)
      .then(response => response.json())
      .then(({totalPages, events})=>{
        setEvents(events);
        setNumberOfPages(totalPages);
