...
function EventList (props) {
  ...
  useEffect(() => {
    if(props.name==="up")
      {
      fetch(`https://cn-events.herokuapp.com/events/up?page=${pageNumber}`)
      ...
      {
      fetch(`https://cn-events.herokuapp.com/events/ar?page=${pageNumber}`)
      .then(response => response.json())
      .then(({totalPages, events})=>{
        setEvents(events);
        setNumberOfPages(totalPages);
      ...
      {
      fetch(`https://cn-events.herokuapp.com/events/fav?page=${pageNumber}`)
      .then(response => response.json())
      .then(({totalPages, events})=>{
        setEvents(events);
        setNumberOfPages(totalPages);
